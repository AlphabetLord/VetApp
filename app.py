...
# ──────────────────────────────────────────────────────────────────────────────
# GEMINI CLIENT (cached so we don't recreate on every rerun)
# ──────────────────────────────────────────────────────────────────────────────
@st.cache_resource(show_spinner=False)
def get_gemini_client() -> genai.Client:
    """Return a reusable Gemini client authenticated via Streamlit secrets."""
        ...
        with col_analysis_engine:
            ...
            if trigger_pipeline:
                with st.spinner("Processing deep reasoning pipeline calculations..."):
                    # ──────────────────────────────────────────────────────────
                    # LIVE GEMINI API CALL — replaces mock generation block
                    # ──────────────────────────────────────────────────────────
                    try:
                        client = get_gemini_client()

                        # Reset file pointer and open as PIL Image
                        uploaded_scan.seek(0)
                        pil_image = Image.open(uploaded_scan)

                        # Build the contextual prompt that bundles patient data,
                        # scan parameters, and any custom driver overrides
                        context_prompt = f"""PATIENT CONTEXT:
- Patient Name: {active_pt['name']}
- Species / Breed: {active_pt['breed']}
- Weight: {active_pt['weight']} kg
- Primary Clinical Indication: {active_pt['condition']}
- Clinical Alerts / History: {active_pt['notes']}

SCAN PARAMETERS:
- Modality: {scan_modality}
- Projection: {scan_projection}

ADDITIONAL DRIVER CONTEXT:
{custom_driver_override if custom_driver_override else 'Standard full field evaluation — no specific focus override.'}
                ...
                with col_d2:
                    ...
                    <body style="font-family:sans-serif; color:#0f172a; padding:40px;">
                        ...
                        <div style="background:#f8fafc; padding:20px; border-radius:8px; line-height:1.6;">
                            {latest_report['content'].replace(chr(10), '<br>')}
                        </div>
                    </body>
                    </html>
                    """
...
# TAB 3: ARCHIVED REPORT REPOSITORY (ENTERPRISE CLINIC HISTORY STORAGE MANAGEMENT LAYER)
with tab_database:
